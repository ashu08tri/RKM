...
const OurVisionSection = ({ language }) => {
  ...
  // State for the data from the database
  ...
  // Handle navigation to our-vision-mission-page with the appropriate tab
  const navigateToVisionPage = (tab) => {
    // Map the activeTab to the corresponding tab ID in the our-vision-mission-page
    const tabMapping = {
      media: 'media',
      programs: 'programs',
      projects: 'upcoming'
  ...
  const getActiveTabData = () => {
    switch (activeTab) {
      case 'media':
        return mediaItems.length > 0 ? mediaItems.slice(0, 6) : []; // Show more items in carousel
      case 'programs':
        return programs.length > 0 ? programs.slice(0, 6) : [];
      case 'projects':
  ...
  // Get the appropriate data for the expanded view
  const getExpandedViewData = () => {
    switch (expandedView) {
      case 'media':
        return mediaItems;
      case 'programs':
  ...
  console.log('Current tab data:', activeTab, {
          ...
          {tabData.length > 0 ? (
            <div className="px-4">
              <Carousel
                ...
                ssr={true}
              >
                {tabData.map(item => renderItem(item))}
              </Carousel>
              <style jsx global>{`
                .carousel-container {
                  padding-bottom: 40px;
